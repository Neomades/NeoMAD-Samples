...
import com.neomades.io.sensor.categories.MagnetometerData;
...
/**
 * Displays data coming from the magnetometer sensor updating the screen at a
 * rate corresponding to the chosen speed acquisition.
 */
public class MagnetometerExampleScreen extends Screen implements SensorListener {
	...
	protected void onCreate() {
		...
		setTitle(Res.string.TITRE_MAGNETOMETER);
		...
		// Checks that a magnetometer is available
		// Note: Make sure before using a sensor that the sensor is available on
		// the device
		if (manager.isMagnetometerSupported()) {
			// Creation of a magnetometer
			magnetometer = manager.getMagnetometer();
		...
		if (!sensorAvailable) {
			...
			((TextLabel) findView(Res.id.ERROR_SENSOR_LABEL)).setText(Res.string.ERROR_MAGNETOMETER);
			...
			((TextLabel) findView(Res.id.DESCRIPTION_LABEL)).setText(Res.string.DESCRIPTION_MAGNETOMTER);
			...
			// Starts magnetometer when "Start" button is pushed
			((Button) findView(Res.id.START_BUTTON)).setClickListener(new ClickListener() {
				public void onClick(View view) {
					if (magnetometer != null) {
						magnetometer.start(period);
					}

				}
			});

			// Stops magnetometer when "Stop" button is pushed
			...
			((ComboBox) findView(Res.id.COMBO)).setItemSelectedListener(new ItemSelectedListener() {
				...
				public void onItemSelected(int index, String item) {
					...
					// want to start the magnetometer.
					...
					// Checks if a magnetometer is currently running.
	...
	public void onSensorChanged(Sensor sensor, SensorData data) {
		// Creation of MagnetometerData to get values from the magnetometer
		final MagnetometerData magnetometerData = (MagnetometerData) data;
		// By default sensor acquisition is done in a background thread
		// If you need to update the UI use runOnUiThread()
		controller.runOnUiThread(new Runnable() {

			public void run() {
				// Gets the last values from the magnetometer. Updates the text
				// labels in the UI thread.
				((TextLabel) findView(Res.id.X_LABEL)).setText(ResManager.getString(Res.string.X_) + Double.toString(magnetometerData.getX()));
				((TextLabel) findView(Res.id.Y_LABEL)).setText(ResManager.getString(Res.string.Y_) + Double.toString(magnetometerData.getY()));
				((TextLabel) findView(Res.id.Z_LABEL)).setText(ResManager.getString(Res.string.Z_) + Double.toString(magnetometerData.getZ()));
			}
		});
